...
import com.don2.shopintelli.databinding.ActivityRegisterCamisetaBinding
import com.google.android.material.chip.Chip
import org.json.JSONObject

class RegisterCamisetaActivity : AppCompatActivity() {
    private lateinit var binding: ActivityRegisterCamisetaBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityRegisterCamisetaBinding.inflate(layoutInflater)
            ...
            if(id_loja != "No imported" && !TextUtils.isEmpty(binding.productName.text) &&
                ...
                for (tam in list_sizes) {
                    for (color in list_colors) {
                        ...
                        json.put("size_camiseta", tam)
                        json.put("color", color)
                        json.put("type", "CAMISETA")
    ...
    private fun sendClient(json: JSONObject) {
        ...
        val url = "https://backend-proj-app-vc5xcezzwa-uc.a.run.app/api/v1/camiseta"
